...
// The default type converter has a schema mismatch for NetworkPolicy types that
// causes SSA to fail. Using the deduced converter avoids this.
...
func TestResourceName(t *testing.T) {
	tt := []struct {
		...
		},
		{
			...
			expected: "this-is-a-very-long-catalog-source-na-6e764be4-lifecycle-server",
		},
		{
		...
		},
		{
			...
			expected: "this-is-a-very-long-catalog-source-na-58089d1c-lifecycle-server",
		},
		{
...
func TestPodStartedAfter(t *testing.T) {
	t1 := metav1.Now()
	t2 := metav1.NewTime(t1.Add(1))

	podWithStart := func(name string, ts metav1.Time) *corev1.Pod {
		return &corev1.Pod{
			ObjectMeta: metav1.ObjectMeta{Name: name},
			Status:     corev1.PodStatus{StartTime: &ts},
		}
	}
	...
	tt := []struct {
		...
		{
			name:     "a started after b returns true",
			a:        podWithStart("a", t2),
			b:        podWithStart("b", t1),
			expected: true,
		},
		{
			name:     "a started before b returns false",
			a:        podWithStart("a", t1),
			b:        podWithStart("b", t2),
			expected: false,
		},
		{
			name:     "equal timestamps returns false",
			a:        podWithStart("a", t1),
			b:        podWithStart("b", t1),
			expected: false,
		},
		{
			name: "falls back to CreationTimestamp when StartTime is nil",
			a: &corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{Name: "a", CreationTimestamp: t2},
			},
			b: &corev1.Pod{
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			require.Equal(t, tc.expected, podStartedAfter(tc.a, tc.b))
		})
	}
}
...
func requireOwnerRef(t *testing.T, refs []metav1ac.OwnerReferenceApplyConfiguration, cs *operatorsv1alpha1.CatalogSource) {
	t.Helper()
	require.Len(t, refs, 1)
	ref := refs[0]
	require.Equal(t, operatorsv1alpha1.SchemeGroupVersion.String(), *ref.APIVersion)
	require.Equal(t, operatorsv1alpha1.CatalogSourceKind, *ref.Kind)
	require.Equal(t, cs.Name, *ref.Name)
	require.Equal(t, cs.UID, *ref.UID)
...
func TestBuildServiceAccount(t *testing.T) {
	...
	requireOwnerRef(t, sa.ObjectMetaApplyConfiguration.OwnerReferences, cs)
}
...
func TestBuildService(t *testing.T) {
	...
	requireOwnerRef(t, svc.ObjectMetaApplyConfiguration.OwnerReferences, cs)
}
...
func TestBuildDeployment(t *testing.T) {
	tt := []struct {
		...
		},
		{
			...
			cs: &operatorsv1alpha1.CatalogSource{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "custom-catalog",
					Namespace: "test-ns",
					UID:       types.UID("test-uid-custom-catalog"),
				},
				Spec: operatorsv1alpha1.CatalogSourceSpec{
		...
		{
			name: "relative catalog dir is joined correctly",
			cs: &operatorsv1alpha1.CatalogSource{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "relative-catalog",
					Namespace: "test-ns",
					UID:       types.UID("test-uid-relative-catalog"),
				},
				Spec: operatorsv1alpha1.CatalogSourceSpec{
					GrpcPodConfig: &operatorsv1alpha1.GrpcPodConfig{
						ExtractContent: &operatorsv1alpha1.ExtractContentConfig{
							CatalogDir: "relative/path",
						},
					},
				},
			},
			imageRef:           "sha256:ghi789",
			nodeName:           "",
			expectedCatalogDir: "/catalog/relative/path",
		},
	}
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			requireOwnerRef(t, deploy.ObjectMetaApplyConfiguration.OwnerReferences, tc.cs)
		})
	}
}
...
func TestBuildNetworkPolicy(t *testing.T) {
	...
	requireOwnerRef(t, np.ObjectMetaApplyConfiguration.OwnerReferences, cs)
}
...
func TestReconcile_CatalogSourceDoesNotMatchSelectors(t *testing.T) {
	...
	// CRB should be reconciled (exists with no subjects since nothing matches)
	var crb rbacv1.ClusterRoleBinding
	err = cl.Get(context.Background(), types.NamespacedName{Name: clusterRoleBindingName}, &crb)
	require.NoError(t, err)
	require.Empty(t, crb.Subjects)

	// Non-matching CatalogSource should NOT get a finalizer
	var updatedCS operatorsv1alpha1.CatalogSource
	err = cl.Get(context.Background(), types.NamespacedName{Name: "test-catalog", Namespace: "test-ns"}, &updatedCS)
	require.NoError(t, err)
	require.NotContains(t, updatedCS.Finalizers, finalizerName)
}

func TestReconcile_CatalogSourceStopsMatching_RemovesFinalizer(t *testing.T) {
	ctx := context.Background()

	// CatalogSource that previously matched (has our finalizer) but labels have changed
	cs := newCatalogSource("test-catalog", "test-ns", map[string]string{"env": "dev"})
	...
	// Finalizer should be removed since the CatalogSource no longer matches
	var updatedCS operatorsv1alpha1.CatalogSource
	err = cl.Get(ctx, types.NamespacedName{Name: "test-catalog", Namespace: "test-ns"}, &updatedCS)
	require.NoError(t, err)
	require.NotContains(t, updatedCS.Finalizers, finalizerName)

	// CRB should be reconciled
	var crb rbacv1.ClusterRoleBinding
...
func TestReconcile_MatchingCatalogSourceWithRunningPod(t *testing.T) {
	...
	// Verify finalizer was added to CatalogSource
	var updatedCS operatorsv1alpha1.CatalogSource
	err = cl.Get(ctx, types.NamespacedName{Name: "test-catalog", Namespace: "test-ns"}, &updatedCS)
	require.NoError(t, err)
	require.Contains(t, updatedCS.Finalizers, finalizerName)
}
...
func TestReconcileClusterRoleBinding(t *testing.T) {
	...
	t.Run("no matching CatalogSources produces CRB with no subjects", func(t *testing.T) {
		...
		err := r.reconcileClusterRoleBinding(ctx)
		require.NoError(t, err)
...
func TestGetCatalogPodInfo_PicksNewestPod(t *testing.T) {
	ctx := context.Background()

	t1 := metav1.Now()
	t2 := metav1.NewTime(t1.Add(1))

	olderPod := catalogPod("test-catalog", "test-ns", "worker-old", "sha256:old-digest", corev1.PodRunning)
	olderPod.Name = "test-catalog-pod-old"
	olderPod.Status.StartTime = &t1

	newerPod := catalogPod("test-catalog", "test-ns", "worker-new", "sha256:new-digest", corev1.PodRunning)
	newerPod.Name = "test-catalog-pod-new"
	newerPod.Status.StartTime = &t2

	cl := testClientBuilder().
		WithObjects(olderPod, newerPod).
		Build()
	...
	image, node, err := r.getCatalogPodInfo(ctx, cs)
	require.NoError(t, err)
	require.Equal(t, "sha256:new-digest", image)
	require.Equal(t, "worker-new", node)
}
...
// --- Reconcile deletion test ---

func TestReconcile_CatalogSourceDeleted_ReconcilesCRBAndRemovesFinalizer(t *testing.T) {
	ctx := context.Background()

	// CatalogSource with our finalizer (simulates a previously reconciled CS)
	cs := newCatalogSource("test-catalog", "test-ns", nil)
	cs.Finalizers = []string{finalizerName}
	...
	// Delete the CatalogSource — fake client sets DeletionTimestamp because the finalizer is present
	err := cl.Delete(ctx, cs)
	require.NoError(t, err)

	// Reconcile handles the finalizer: reconciles CRB and removes the finalizer
	result, err := r.Reconcile(ctx, ctrl.Request{
		NamespacedName: types.NamespacedName{Name: "test-catalog", Namespace: "test-ns"},
	})
	require.NoError(t, err)
	...
	// Verify finalizer was removed (CatalogSource should now be fully gone)
	err = cl.Get(ctx, types.NamespacedName{Name: "test-catalog", Namespace: "test-ns"}, &operatorsv1alpha1.CatalogSource{})
	require.True(t, errors.IsNotFound(err), "CatalogSource should be fully deleted after finalizer removal")
}
...
func TestReconcile_DeploymentDetails(t *testing.T) {
	...
	// Tolerations
	require.Len(t, podSpec.Tolerations, 4)
	...
	require.Equal(t, "node-role.kubernetes.io/control-plane", podSpec.Tolerations[1].Key)
	require.Equal(t, corev1.TaintEffectNoSchedule, podSpec.Tolerations[1].Effect)
	require.Equal(t, "node.kubernetes.io/unreachable", podSpec.Tolerations[2].Key)
	require.Equal(t, corev1.TaintEffectNoExecute, podSpec.Tolerations[2].Effect)
	require.NotNil(t, podSpec.Tolerations[2].TolerationSeconds)
	require.Equal(t, int64(120), *podSpec.Tolerations[2].TolerationSeconds)
	require.Equal(t, "node.kubernetes.io/not-ready", podSpec.Tolerations[3].Key)
	...
	require.Equal(t, resource.MustParse("200Mi"), memLimit)
...
// --- ensureResources: partial Apply failure ---

func TestEnsureResources_PartialApplyFailure(t *testing.T) {
	ctx := context.Background()
	...
	err := r.ensureResources(ctx, cs, "sha256:abc123", "worker-1")
	require.Error(t, err)
	require.ErrorIs(t, err, applyErr)

	// First resource (ServiceAccount) should have been created before the failure
	var sa corev1.ServiceAccount
	err = cl.Get(ctx, types.NamespacedName{Name: name, Namespace: "test-ns"}, &sa)
	require.NoError(t, err, "ServiceAccount should exist from the successful first Apply")
}

// --- reconcileClusterRoleBinding: non-matching CatalogSources filtered ---

func TestReconcileClusterRoleBinding_NonMatchingCatalogSourcesFiltered(t *testing.T) {
	ctx := context.Background()

	matchingCS := newCatalogSource("matching-catalog", "test-ns", map[string]string{"env": "prod"})
	nonMatchingCS := newCatalogSource("non-matching-catalog", "test-ns", map[string]string{"env": "dev"})

	matchingSAName := resourceName("matching-catalog")
	nonMatchingSAName := resourceName("non-matching-catalog")

	matchingSA := &corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: matchingSAName, Namespace: "test-ns"}}
	nonMatchingSA := &corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: nonMatchingSAName, Namespace: "test-ns"}}

	cl := testClientBuilder().
		WithObjects(matchingCS, nonMatchingCS, matchingSA, nonMatchingSA).
		Build()
	...
	// Only the matching CatalogSource's SA should be a subject
	require.Len(t, crb.Subjects, 1)
	require.Equal(t, matchingSAName, crb.Subjects[0].Name)
}

// --- Reconcile: TLS args appear in deployed container ---

func TestReconcile_TLSArgsInDeployment(t *testing.T) {
	ctx := context.Background()
	...
	r := testReconciler(cl)
	r.TLSConfigProvider = NewTLSConfigProvider(dummyGetCertificate, configv1.TLSProfileSpec{
		MinTLSVersion: configv1.VersionTLS12,
		Ciphers: []string{
			"TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256",
			"TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384",
	...
	for _, arg := range args {
		if strings.HasPrefix(arg, "--tls-min-version=") {
			hasMinVersion = true
			require.Equal(t, "--tls-min-version=VersionTLS12", arg)
		}
		if strings.HasPrefix(arg, "--tls-cipher-suites=") {
			hasCipherSuites = true
		}
	}
	require.True(t, hasMinVersion, "Deployment args should include --tls-min-version")
	require.True(t, hasCipherSuites, "Deployment args should include --tls-cipher-suites for TLS 1.2")
}
