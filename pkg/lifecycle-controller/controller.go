...
func (r *LifecycleServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if imageRef == "" {
		log.Info("no valid image ref for catalog source, cleaning up stale resources")
		if err := r.cleanupResources(ctx, log, cs.Namespace, cs.Name); err != nil {
			return ctrl.Result{}, err
		}
		return ctrl.Result{}, r.reconcileClusterRoleBinding(ctx, log)
...
func (r *LifecycleServerReconciler) cleanupResources(ctx context.Context, log logr.Logger, csNamespace, csName string) error {
	...
	resources := []client.Object{
		&appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: csNamespace}},
		&corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: csNamespace}},
		&networkingv1.NetworkPolicy{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: csNamespace}},
		&corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: csNamespace}},
	}
	...
	for _, obj := range resources {
		...
		if !isOwnedByLifecycleController(obj) {
			log.Info("skipping deletion of resource not owned by lifecycle-controller",
...
// isOwnedByLifecycleController returns true if the resource was created by the lifecycle-controller,
// verified by checking for the app and field-manager labels.
func isOwnedByLifecycleController(obj client.Object) bool {
	labels := obj.GetLabels()
	return labels[appLabelKey] == appLabelVal
}
...
// is too long, a 6-character hash is inserted to prevent collisions between
...
// shortHash returns the first 6 hex characters of the SHA-256 hash of s.
