...
func LoadLifecycleData(fbcPath string, log logr.Logger) (LifecycleIndex, error) {
	...
	err := declcfg.WalkMetasFS(context.Background(), root, func(path string, meta *declcfg.Meta, err error) error {
		if err != nil {
			log.Info("skipping FBC entry due to error", "path", path, "error", err)
			return nil
		}
		if meta == nil {
			return nil
		...
		if _, exists := result[schemaVersion][meta.Package]; exists {
			return fmt.Errorf("duplicate lifecycle blob for version %q package %q in file %q", schemaVersion, meta.Package, path)
		}
...
// CountPackages returns the number of unique packages across all versions
