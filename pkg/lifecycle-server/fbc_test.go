...
func TestLoadLifecycleData(t *testing.T) {
	tt := []struct {
		...
		{
			name: "corrupted entries are logged and skipped, valid entries still loaded",
			setup: func(t *testing.T) string {
				dir := t.TempDir()
			...
			// logs and skips them. No error is returned overall, and valid entries
			// from other files are still loaded successfully.
			expectedIndex: LifecycleIndex{
				"v1alpha1": {
		...
		{
			name: "duplicate lifecycle blob returns error",
			setup: func(t *testing.T) string {
				dir := t.TempDir()
				writeFBCFile(t, dir, "catalog.json",
					map[string]any{
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			path := tc.setup(t)
			result, err := LoadLifecycleData(path, logr.Discard())
			if tc.expectErr {
				require.Error(t, err, "expected LoadLifecycleData to return an error")
				return
			}
			require.NoError(t, err, "LoadLifecycleData returned unexpected error")
			...
			for version, expectedPkgs := range tc.expectedIndex {
				...
				for pkg, expectedBlob := range expectedPkgs {
					...
					require.NoError(t, json.Unmarshal(expectedBlob, &expectedMap), "failed to unmarshal expected blob for package %q", pkg)
					require.NoError(t, json.Unmarshal(resultBlob, &resultMap), "failed to unmarshal result blob for package %q", pkg)
					require.Equal(t, expectedMap, resultMap, "blob content mismatch for version %q package %q", version, pkg)
				}
			}
		})
	}
...
func TestLifecycleIndex_CountBlobs(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			require.Equal(t, tc.expected, tc.index.CountBlobs(), "CountBlobs mismatch")
		})
	}
}
...
func TestLifecycleIndex_CountPackages(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			require.Equal(t, tc.expected, tc.index.CountPackages(), "CountPackages mismatch")
		})
	}
}
...
func TestLifecycleIndex_ListVersions(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.expected, result, "ListVersions mismatch")
		})
	}
}
...
func TestLoadLifecycleData_Subdirectory(t *testing.T) {
	...
	require.NoError(t, err, "failed to create nested test directory")
	...
	result, err := LoadLifecycleData(dir, logr.Discard())
	require.NoError(t, err, "LoadLifecycleData failed on nested directory")
	require.Contains(t, result, "v1alpha1", "expected v1alpha1 version in result from nested directory")
	require.Contains(t, result["v1alpha1"], "nested-operator", "expected nested-operator package in v1alpha1")
}
